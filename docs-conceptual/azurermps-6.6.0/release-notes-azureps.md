...
* リソースの現在の構成とターゲット リソースの間に互換性があることを検証できるように、Common.Strategy ライブラリを更新しました。 既定値は常に true ですが、個々のリソースによって既定値がオーバーライドされます。
* Common.Storage に ps1xml の種類を追加しました

#### <a name="azurestorage"></a>Azure.Storage
* DefaulfProfile からストレージ コンテキストを取得できるようになりました
